...
/** Every `<owner>/<repo>/<pr>/` holding `marker` under `root`. A suite that finds none has a broken path, not zero cases. */
export function discoverCases(
  root: URL,
  marker: string,
): { owner: string; repo: string; pr: number; dir: string }[] {
  const base = fileURLToPath(root);
  const cases = globSync(`*/*/*/${marker}`, { cwd: base })
    .sort()
    .map((hit) => {
