...
export async function predict(
  ...
  // Execution is on by default: the sandboxed executor is how the values a
  // dynamic matrix depends on get learned, and it costs nothing until a
  // workflow actually needs one. `executor: null` turns execution off; a
  // given executor stands in for the live one — a test seam, not
  // configuration.
  const executor =
