// The live executor is wiring: tarballs for plain trees, clones for history,
// the sandbox for steps. The wiring is what these tests pin — which provider
// a request routes to, where the clone auth comes from — with the pieces
// themselves already covered by their own suites in `execute.test.ts` and
// `sandbox.test.ts`.
...
/**
 * A real gzipped tarball, `o-r-ccccccc/file.txt` = "content" — the GitHub
 * single-wrapper shape, embedded as bytes like `execute.test.ts` does.
 */
...
describe("makeLiveExecutor", () => {
  it("serves a plain job from the tarball endpoint, end to end", async () => {
    const ex = makeLiveExecutor(octokitOf({ [`o/r@${SHA}`]: WRAPPED_TB }), WORKSPACE, resolveRef, {
      // The host shell stands in for the sandbox: this test is about the tree
      // route, and docker has its own suite.
      runCommand: runShell,
      token: null,
    });
  ...
  it("routes a history request to the clone provider, not the tarball", async () => {
    // The tarball for this commit exists — but `fetch-depth: 0` wants
    // history, which routes to the clone provider, whose remote here does
    // not. Failing proves the routing.
    const ex = makeLiveExecutor(octokitOf({ [`o/r@${SHA}`]: WRAPPED_TB }), WORKSPACE, resolveRef, {
      runCommand: runShell,
      token: null,
