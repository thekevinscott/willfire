...
describe("executing run steps", () => {
  ...
  it("omits the github env seeds when the scope nulls them out", async () => {
    // Contrived: `executeJob` always seeds both facts, so only an explicit
    // undefined reaches this. The seeds must go absent, not "undefined".
    const out = success(
      await execute(
        {
          steps: [
...
describe("composite actions", () => {
  ...
  it("stops on a fetch-depth: 0 checkout hidden inside a composite", async () => {
    // The pre-scan that picks the tree provider reads only the job's own
    // steps, so a checkout inside the action finds a workspace without
    // history — and refuses, rather than serving a shallow tree as deep.
    const tree = await tempTree({
...
describe("node actions", () => {
  ...
  it("seeds empty PATH and HOME and no github vars when the host has none to give", async () => {
    // Contrived on purpose: a host without $PATH and a scope that nulls the
    // seeded github facts. The env the action sees must still be complete —
    // empty strings and absent keys, never the literal "undefined".
...
/**
 * A real git repo with two commits: `main` at commit "one" (f.txt = "a"), and
 * commit "two" (f.txt = "b") parked under `refs/pull/1/head` the way GitHub
 * parks PR heads — reachable from no branch, so a plain clone cannot check it
 * out without fetching it first.
 */
