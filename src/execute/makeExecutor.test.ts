...
describe("executing run steps", () => {
  ...
  it("treats strategy:, container:, and services: left empty as absent", async () => {
  ...
  it("lets only a string uses ask for history in the pre-scan", async () => {
    // A sequence-valued uses coerces to a checkout-shaped string; the
    // pre-scan must not let that coercion request a clone.
...
describe("composite actions", () => {
  ...
  it("refuses an action whose manifest is empty, or has no runs block", async () => {
    // YAML parses an empty manifest to null; both shapes fail the same way.
    const tree = await tempTree({ "a1/action.yml": "", "a2/action.yml": "{}" });
    const ex = executorOf({ [`o/r@${SHA}`]: tree });
    for (const dir of ["./a1", "./a2"]) {
      const o = await ex.executeJob("detect", { steps: [{ uses: dir }] }, {}, {});
      expect(failure(o)).toBe(
        `step '#1': action ${dir} runs via 'undefined'; only composite and node actions are executed`,
      );
    }
  });

  it("matches using: to nodeNN exactly, never as a substring", async () => {
    const tree = await tempTree({
      "a1/action.yml": JSON.stringify({ runs: { using: "node24x" } }),
      "a2/action.yml": JSON.stringify({ runs: { using: "xnode24" } }),
    });
    const ex = executorOf({ [`o/r@${SHA}`]: tree });
    for (const [dir, using] of [
      ["./a1", "node24x"],
      ["./a2", "xnode24"],
    ]) {
      const o = await ex.executeJob("detect", { steps: [{ uses: dir }] }, {}, {});
      expect(failure(o)).toBe(
        `step '#1': action ${dir} runs via '${using}'; only composite and node actions are executed`,
      );
    }
  });

  it("runs a composite with no steps: key as an empty walk", async () => {
    const tree = await tempTree({
      "action/action.yml": JSON.stringify({ runs: { using: "composite" } }),
    });
    const ex = executorOf({ [`o/r@${SHA}`]: tree });
    const o = await ex.executeJob("detect", { steps: [{ uses: "./action" }] }, {}, {});
    expect(success(o)).toEqual({});
...
/**
 * Real gzipped tarballs, embedded so the suite needs no filesystem of its own
 * to produce bytes; extraction below runs the real `tar`.
 */
