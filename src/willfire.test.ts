// End-to-end suite for `willfire()`, driven against a hand-built GitHub client
// stand-in.
//
...
const unserved = (route: string) => (): never => {
  throw new Error(`willfire reached ${route}`);
};
...
function run(body: string, f: Fixture = {}): Promise<Prediction> {
  return willfire(fakeGithub({ contents: { [WF]: body }, ...f }), "o/r", 1);
}
...
describe("willfire", () => {
  ...
  it("ignores anything the Actions API lists outside .github/workflows", async () => {
    ...
    expect(await willfire(github, "o/r", 1)).toEqual({
      entries: [],
      checkNames: [],
      skip: null,
      sources: [HEAD_SOURCE],
  ...
  it("ignores a non-file entry when reading the tree, even one named like a workflow", async () => {
    ...
    expect((await willfire(github, "o/r", 1)).entries).toHaveLength(1);
  });
  ...
  it("fails the prediction when listing the workflow tree fails with anything but a 404", async () => {
    ...
    await expect(willfire(github, "o/r", 1)).rejects.toThrow(
...
describe("the commit workflow files are read at", () => {
  ...
  it("falls back to the head when there is no test merge commit", async () => {
    const { checkNames } = await willfire(fakeGithub({ ...both, mergeSha: null }), "o/r", 1);
...
describe("a caller-supplied event action", () => {
  const runWith = (body: string, f: Fixture, opts: Parameters<typeof willfire>[3]) =>
    willfire(fakeGithub({ contents: { [WF]: body }, ...f }), "o/r", 1, opts);
...
describe("callback commands", () => {
  ...
  it("resolves the commands once and hands every expansion the one map", async () => {
    ...
    vi.mocked(expandJobs).mockClear();
    await willfire(fakeGithub({ contents: { [WF]: BODY } }), "o/r", 1, {
  ...
  it("aborts the prediction when a callback fails", async () => {
    ...
    await expect(
      willfire(fakeGithub({ contents: { [WF]: BODY } }), "o/r", 1, { callbacks: ["r"] }),
...
describe("the executor seam through willfire", () => {
  ...
  const coverEntry = async (f: Fixture, opts: Parameters<typeof willfire>[3] = {}) => {
    const { entries } = await willfire(
  ...
  it("cleans the executor up once, after the last workflow", async () => {
    const order: string[] = [];
    await willfire(fakeGithub({ contents: { [WF]: DYNAMIC } }), "o/r", 1, {
      executor: {
        executeJob: async (jobId) => {
          order.push(jobId);
...
describe("a workflow file that cannot be read", () => {
  ...
  it("fails the prediction when the read fails with anything but a 404 (#177)", async () => {
    ...
    await expect(willfire(rejecting(apiError(503, WF)), "o/r", 1)).rejects.toThrow(
