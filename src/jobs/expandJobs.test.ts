...
describe("with: values across the call boundary", () => {
  ...
  it("passes literals through with their types and falls back to declared defaults", async () => {
    // Job ids mirror the input each guard reads. a–d come from the caller in
    // their literal shapes; e has a default; f is declared with no default and
    // never passed, which is unknown, not empty; g and h are malformed
    // declarations, unknown the same way.
    const entries = await expand(
      {
        call: {
          uses: "./.github/workflows/callee.yml",
