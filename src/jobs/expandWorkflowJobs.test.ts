...
// The executor seam: expansion executes, before anything reads `needs`, the
// jobs some sibling reads outputs from — and folds what an execution yields
// into the scope every later evaluation sees. The executor itself is faked
// here; what it actually does when it runs things is execute.test.ts's subject.
describe("derived execution during expansion", () => {
  ...
  it("reaches a needed job inside a called workflow", async () => {
    // Selection derives from each workflow's own job set, so the recursion
    // is where the callee's producer gets picked up and executed.
