...
/**
 * The block to quote when a command exits non-zero. stderr first, falling back
...
export const failureTail = (r: { stdout: string; stderr: string }): string => {
  const fromStderr = quoteBlock(r.stderr);
  return fromStderr === "" ? quoteBlock(r.stdout) : fromStderr;
};
